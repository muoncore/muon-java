...
/**
 * An implementation of the Muon HTTP TCK Resources to prove compatibility of
 * the library
 */
public class TCKService {
	...
	public static void main(String[] args) throws URISyntaxException,
		...
		final Muon muon = new Muon(new AmqpDiscovery("amqp://localhost:5672"));
	...
	private static void outboundResourcesSetup(final Muon muon) {
		...
		muon.onQuery("/invokeresponse-store", Map.class,
				new MuonService.MuonQuery<Map>() {
					@Override
					public MuonFuture onQuery(MuonResourceEvent<Map> queryEvent) {
						System.out.println("Calling invokeresponse-store");
						return MuonFutures.immediately(storedata);
					}
				});

		muon.onQuery("/invokeresponse", Map.class,
				new MuonService.MuonQuery<Map>() {
					@Override
					public MuonFuture onQuery(MuonResourceEvent<Map> queryEvent) {
						System.out.println("Calling invokeresponse");

						String url = (String) queryEvent.getDecodedContent()
								.get("resource");

						MuonFuture<MuonClient.MuonResult<Map>> rsult = muon
								.get(url, Map.class);

						return MuonFutures.fromPublisher(Streams
								.wrap(rsult.toPublisher())
								.map(new Function<MuonClient.MuonResult<Map>, Map>() {
									@Override
									public Map apply(
											MuonClient.MuonResult<Map> mapMuonResult) {
										Map data = mapMuonResult
												.getResponseEvent()
												.getDecodedContent();
										storedata.clear();
	...
	private static void inboundResourcesSetup(final Muon muon) {
		...
		muon.onCommand("/echo", Map.class, new MuonService.MuonCommand<Map>() {
			...
			public MuonFuture<Map> onCommand(MuonResourceEvent<Map> queryEvent) {
				System.out.println("Calling command on /echo");
		...
		muon.onQuery("/discover", Map.class, new MuonService.MuonQuery<Map>() {
			@Override
			public MuonFuture<List<String>> onQuery(
					MuonResourceEvent<Map> queryEvent) {
				System.out.println("Calling query on /discover");
	...
	private static List<Map> broadcastSetup(final Muon muon) {
		final List<Map> events = Collections
				.synchronizedList(new ArrayList<Map>());

		muon.receive("echoBroadcast", Map.class,
				new MuonService.MuonListener<Map>() {
					public void onEvent(MuonMessageEvent<Map> event) {
						System.out.println("Calling echoBroadcast");
						muon.emit(MuonMessageEventBuilder
								.named("echoBroadcastResponse")
								.withContent(event.getDecodedContent()).build());
					}
				});

		muon.receive("tckBroadcast", Map.class,
				new MuonService.MuonListener<Map>() {
					public void onEvent(MuonMessageEvent<Map> event) {
						System.out.println("Calling tckBroadcast");
		...
		muon.onQuery("/event", Map.class, new MuonService.MuonQuery<Map>() {
			@Override
			public MuonFuture onQuery(MuonResourceEvent<Map> queryEvent) {
				System.out.println("Calling query on /event");
		...
		muon.onCommand("/event", Map.class, new MuonService.MuonCommand<Map>() {
			...
			public MuonFuture onCommand(MuonResourceEvent<Map> queryEvent) {
				System.out.println("Calling command on /event");
				if(queryEvent.getDecodedContent().get("delete").equals("all")) {
	...
	private static void queueSetup(final Muon muon) {
		// Setup required to pass the Queue TCK. Not convinced this concept is a
		// good idea at all.
		final List<Map> queueEvents = Collections
				.synchronizedList(new ArrayList<Map>());

		muon.onQueue("tckQueue", Map.class,
				new MuonService.MuonListener<Map>() {
					@Override
					public void onEvent(MuonMessageEvent<Map> event) {
						System.out.println("Calling tckQueue");
						queueEvents.clear();
						queueEvents.add(event.getDecodedContent());
					}
				});
		muon.onQueue("tckQueueSend", Map.class,
				new MuonService.MuonListener<Map>() {
					@Override
					public void onEvent(MuonMessageEvent<Map> event) {
						System.out.println("Calling tckQueueSend");
						Map data = event.getDecodedContent();
						muon.sendMessage(MuonMessageEventBuilder
								.named((String) data.get("data"))
								.withNoContent().build());
					}
				});

		muon.onQuery("/tckQueueRes", Map.class,
				new MuonService.MuonQuery<Map>() {
					@Override
					public MuonFuture onQuery(MuonResourceEvent<Map> queryEvent) {
						System.out.println("Calling tckQueueRes");
